...
pub fn print_response_with_opts(resp: &Response, action: Option<&str>, opts: &OutputOptions) {
    ...
    if let Some(data) = &resp.data {
        ...
        if let Some(tabs) = data.get("tabs").and_then(|v| v.as_array()) {
            for tab in tabs {
                let tab_id = tab.get("tabId").and_then(|v| v.as_i64()).unwrap_or_default();
                let title = tab
                    .get("title")
                    .and_then(|v| v.as_str())
                    .unwrap_or("Untitled");
        ...
        if let Some(tab_id) = data.get("tabId").and_then(|v| v.as_i64()) {
            if let Some(total) = data.get("total").and_then(|v| v.as_i64()) {
                ...
                println!("{} {} [{}] ({} total)", color::success_indicator(), label, tab_id, total);
                return;
            }
        }
        // Console logs
...
Operations:
  ...
  new [url]            Open new tab and print its tab ID
  close [tab-id]       Close tab by tab ID (current if omitted)
  <tab-id>             Switch to tab by tab ID
...
Tabs:
  tab [new|list|close|<id>]  Manage tabs
